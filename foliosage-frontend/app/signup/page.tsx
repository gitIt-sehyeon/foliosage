...
import { ArrowRight, FileCheck2, MessageSquareText, ShieldCheck, Sparkles } from 'lucide-react'
...
export default function SignupPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#060912] text-white">
      ...
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_18%,rgba(124,58,237,0.22),transparent_28%),radial-gradient(circle_at_76%_32%,rgba(245,158,11,0.10),transparent_24%),radial-gradient(circle_at_62%_86%,rgba(20,184,166,0.12),transparent_24%)]" />
      ...
      <section className="relative z-10 mx-auto grid min-h-[calc(100vh-76px)] max-w-6xl items-center gap-10 px-5 pb-16 sm:px-8 lg:grid-cols-[1fr_440px]">
        <div className="hidden max-w-xl animate-slide-up lg:block">
          <p className="mb-4 text-xs font-medium uppercase tracking-[0.28em] text-cyan-200">Start with the archive</p>
          <h1 className="text-5xl font-semibold leading-tight">Turn scattered work into a portfolio that speaks.</h1>
          <div className="mt-8 grid grid-cols-3 gap-3">
            {[
              { icon: FileCheck2, label: 'Organize' },
              { icon: ShieldCheck, label: 'Certify' },
              { icon: MessageSquareText, label: 'Answer' },
            ].map(item => (
        ...
        <div className="animate-spring-in rounded-2xl border border-white/10 bg-[#0b1020]/90 p-6 shadow-2xl shadow-black/40 backdrop-blur-xl sm:p-8">
          <div className="mb-7">
            <p className="text-sm text-violet-200">Create your workspace</p>
            <h2 className="mt-2 text-3xl font-semibold text-white">Join FolioSage</h2>
            <p className="mt-2 text-sm text-slate-500">Build a public portfolio with file proof and AI context.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <p className="text-center text-sm text-slate-500">
              Already have an account? <Link href="/login" className="text-violet-200 hover:text-white">Log in</Link>
            </p>
          </form>
        </div>
      </section>
